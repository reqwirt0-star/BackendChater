// server/server.js - ПОЛНАЯ ВЕРСИЯ С API ДЛЯ ДЕТАЛЬНОЙ СТАТИСТИКИ
...
app.post('/api/track-click', async (req, res) => {
    ...
    if (!username) {
        return res.status(401).json({ message: 'Анонимный клик' });
    }
    if (!buttonId) {
        return res.status(400).json({ message: 'Не указан ID кнопки' });
    }
    try {
        ...
        console.error('Ошибка при записи клика:', error.response ? error.response.data : error.message);
        res.status(500).json({ message: 'Ошибка на сервере при записи клика' });
    }
});
...
app.get('/api/analytics', async (req, res) => {
    ...
    try {
        ...
        const { data: stats, error } = await axios.post(
            `${SUPABASE_URL}/rest/v1/rpc/get_detailed_analytics`,
